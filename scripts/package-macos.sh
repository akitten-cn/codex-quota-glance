...
version="$(cargo metadata --no-deps --format-version 1 | /usr/bin/python3 -c 'import json,sys; print(next(p["version"] for p in json.load(sys.stdin)["packages"] if p["name"] == "codex-taskbar"))')"
...
cp prototypes/macos-layout.css "$app/Contents/Resources/"
